    ...
    columns:[
        ...
        {data: null,
            "render": function(data){
              if(data.estado == 0){
                return `<label class="switch" onchange="cambiarEstado(${data.id}, ${data.estado})">
        ...
        {data: null,
            "render": function(data){
                return `<a style="text-align: center;" onclick="editar_modal(${data.id},'${data.descripcion}')"><span class="material-icons-sharp action_item" id="edit_button">edit</span></a> <a onclick="confirmar(${data.id})"><span class="material-icons-sharp action_item eliminar_button">delete</span></a>`
            }
        }
    ],
...
function confirmar(id){
    Swal.fire({
        ...
        if (result.isConfirmed) {
          window.location = '/producto/deleteUnidad/' + id;
...
openButton.addEventListener("click", function() {
  ...
  $("#modal_titulo").text("Agregar Unidad");
  $('form').attr('action', 'save_unidad');
});
...
function editar_modal(id , descripcion){ 
  ...
  $("#modal_titulo").text("Editar Unidad");
  $('form').attr('action', 'update_unidad');
}
...
function cambiarEstado(id, estado){
    ...
    if(estado == 0){
        ...
        window.location = '/producto/update_estado_unidad/' + id + '/' + valor;
    }else if(estado == 1){
        console.log(estado);
        valor = estado - 1;
        /*console.log($("#togBtn").val())
