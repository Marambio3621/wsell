    ...
    columns:[
        ...
        {data: null,
            "render": function(data){
                if(data.tipo == "v"){
                    return `<h3 style="color: var(--color-white);font-size:1.1rem; border: 1px solid black; border-radius: var(--border-radius-1); text-align:center; background-color:var(--color-primary)">Vendedor</h3>`
                }else{
                    return `<h3 style="color: var(--color-white);font-size:1.1rem; border: 1px solid black; border-radius: var(--border-radius-1); text-align:center; background-color:var(--color-warning)">Administrador</h3>`
                } 
            }
        },
        {data: null,
            "render": function(data){
                if(data.estado == 0){
                    return `<label class="switch" onchange="cambiarEstado(${data.idusuario}, ${data.estado})">
                    ...
                    return `<label class="switch"  onchange="cambiarEstado(${data.idusuario}, ${data.estado})">
                            ...
                            <span class="slider round marcado"></span>
                        </label>`
                }
        ...
        {data: null,
            "render": function(data){
                return `<a 
                onclick="editar_modal(${data.idusuario},'${data.nombre}','${data.apellido_paterno}','${data.apellido_materno}','${data.correo}', '${data.telefono}' ,'${data.direccion}', '  ${data.tipo}')">
                <span class="material-icons-sharp action_item" id="edit_button">edit</span>
                </a> 
                <a onclick="confirmar(${data.idusuario})">
                <span class="material-icons-sharp action_item eliminar_button">delete</span>
                </a>
                <a onclick="reset_password(${data.idusuario})">
                <span class="material-icons-sharp action_item reset_password">restart_alt</span>
                </a>`
            }
        }
    ],
...
openButton.addEventListener("click", function() {
  ...
  $("#modal_titulo").text("Agregar Usuario");
...
function editar_modal(idusuario, nombre , apellido_p, apellido_m, correo, telefono, direccion, tipo){
  modal.classList.toggle("closed");
  modalOverlay.classList.toggle("closed");
  ...
  $('form').attr('action', 'updateUsuario');
}

var submitButton = document.querySelector("#btn_submit");
submitButton.addEventListener("click", function() {
    if( $('#rut').val() == ''|| $('#nombre').val() == '' || $('#apellido_p').val() == '' || $('#apellido_m').val() == '' || $('#correo').val() == '' || $('#telefono').val() == '' || $('#direccion').val() == '' || $('#tipo').val() == ''){
        Swal.fire({
...
//FUNCION PARA RESETEAR LA CONTRASEÑA POR DEFECTO DE WSELL
function reset_password(id){
    Swal.fire({
        title: '¿Confirma reestablecer la contraseña?',
        icon: 'warning',
        showCancelButton: true,
        confirmButtonColor: 'var(--color-primary)',
        cancelButtonColor: 'var(--color-danger)',
...
function cambiarEstado(rut, estado){
    estado;
    if(estado == 0){
        ...
        window.location = '/usuarios/update_estado_usuario/' + rut + '/' + valor;
    }else if(estado == 1){
        console.log(estado);
        valor = estado - 1;
        /*console.log($("#togBtn").val())
